<template>
  <div>
    <el-dialog :title="info.isAdd?'菜单添加':'菜单编辑'" :visible.sync="info.isShow" @close="cancel" @opened="opened">
      <el-form ref="form" :model="form" label-width="80px">
        ...
        <el-form-item label="规格属性">
          <el-select v-model="form.specsattr" multiple placeholder="请选择规格属性">
            <el-option v-for="item in specsArr" :key="item.id" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      // specs列表请求
      reqSpecsList: "specslist/reqChangeList",
      // goods列表请求
      reqGoodsList: "goodslist/reqChangeList",
      // goods列表总条数
      reqGoodsTotal: "goodslist/reqChangeTotal",
    }),
    // 当打开动画完成时
    opened() {
      // 加载富文本
    ...
    // 当一级分类发生变化的时候
    // changeFirst() {
    //   catelist().then(async (res) => {
    //     // var list = await res.data.list;
